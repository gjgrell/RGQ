...
def R_analytic_global(Z, v, mixing, gamma, alpha, R0, R, L, beta, ximax):
    ...
    Li_s_params = get_params_Li(Z, 's')
    s_centroid_energy = df.loc[Z,'s']
    fac_s_osc_strength = Li_s_params[1]
    s_lorentz_gamma = Li_s_params[2] * hbar #eV

    #Li-like t absorbing
    Li_t_params = get_params_Li(Z, 't')
    t_centroid_energy = df.loc[Z,'t']
    fac_t_osc_strength = Li_t_params[1]
    t_lorentz_gamma = Li_t_params[2] * hbar #eV
	...
	#Form 3D meshgrid for integration
    theta_grid, phi_grid, r_grid = np.meshgrid(theta_lin, phi_lin, r_lin) 
	...
	#xi - maximum ionization parameter
    xi_max = ximax 
    ...
    #____
    #tot_beta_Li, tot_beta_He = photoionization cross sections * ionizing radiation
    egrid_Li = []
    pi_sig_Li = []
...
def Q_analytic_global(Z, v, mixing, gamma, alpha, R0, R, L, beta, ximax):
...
def G_analytic_local(Z, N_li, N_he, v, pi, nele, mixing, gamma):
  ...
  #____
    ...
    #Critical UV photoexcitation rate
    phi_c = Azg / (1 + F) #s-1
...
def G_analytic_global(Z, v, mixing, gamma, alpha, R0, R, L, beta, ximax):
    ...
    G = (1 / K) * (Upsilon / (Upsilon + 1)) * (1 + F + S - B * (1 - tilde_beta_He / bar_beta_He))  
    return float(G)
