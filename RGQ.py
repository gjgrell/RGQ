...
def R_analytic_obs(Z, N_li, N_he, v, mixing, gamma, alpha, R0, R, L, beta, xi_max):
  ...
  #____
    ...
    if beta == 2:
    	...
    	Froot = F_root(L,r0=R0, r=r, beta=beta, xi_max=xi_max)
...
#Assuming simple, truncated cone for geometry
#Assumming cold photoionized plasma (ne << nc, phi_UV << phi_c) 
def R_analytic_global_old(Z, v, mixing, gamma, alpha, R0, R, L, beta, xi_max):
    ...
    Froot_he_high_log = 10**(Froot_he_high)
    Froot_he_low_log = 10**(Froot_he_low)

    Froot_he = (Froot_he_low_log + Froot_he_high_log) / 2
    ...
    Froot_li_high_log = 10**(Froot_li_high)
    Froot_li_low_log = 10**(Froot_li_low)

    Froot_li = (Froot_li_low_log + Froot_li_high_log) / 2

    n_li_low = 10**(Froot_li_low) * A_si * n0_low * (R0/r_grid)**(beta) 
    n_li_high = 10**(Froot_li_high) * A_si * n0_high * (R0/r_grid)**(beta) 

    #Take average for Li-like ion density
...
def G_analytic_obs(Z, N_li, N_he, v, mixing, gamma, alpha, R0, R, L, beta, xi_max):
    #____
    #Relevant ratios F, B, K for analytic R ratio calculation
    ratios = get_ratios(Z, 0)
    F = ratios[0] #Ratio of collisional population of z / (x + y)
  ...
  #____
    ...
    for i in range(len(e_grid)):
        ...
        #Integrate over volume (spherical coordinates)
        E_integrand[i] = simpson(simpson(simpson((np.exp(-1 * tau) / (r/R0)**(beta)) * np.sin(theta) * 10**(Froot), theta_lin, axis = 0), phi_lin, axis = 0), r_lin, axis = 0)    
        
    #Integrate spatial integral A_0 (component without absorption) over volume
    A_0 = simpson(simpson(simpson((1 / (r/R0)**(beta)) * np.sin(theta) * 10**(Froot), theta_lin, axis = 0), phi_lin, axis = 0), r_lin, axis = 0)
    ...
    #E = tot_beta_He * simpson(simpson(simpson(10**(Froot) * (R0/r_grid)**(beta) * np.sin(theta_grid) * (Btilde / B), theta_lin, axis=0),phi_lin,axis=0),r_lin,axis=0)    
...
def chi_for_min_global(x, Z, gamma, alpha, L, inc, RGQ_meas, RGQ_meas_err):
    print(x)
    vel = x[0]
    mixing = x[1]
    R0 = x[2]
...
def solve_RGQ_global(Z, gamma, alpha, L, inc, RGQ_meas, RGQ_meas_err):
    res = minimize(chi_for_min_global,x0=[250, 0.5, 0.4, 1.5, 10.0],args=(Z, gamma, alpha, L, inc, RGQ_meas,RGQ_meas_err),bounds=[(100.1,399.9), (0.01, 0.99), (0.31, 0.51), (1.01, 1.99),(9.01, 11.01)])
    return res
