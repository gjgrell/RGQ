...
#THESE VARIABLES MUST BE WRITTEN AS DICTS
Z = {
    "Si": "Si",
    "Mg": "Mg",
    "S": "S",
...
RGQ_meas = {
    "Si": np.array([3.89, 1.37, 0.42]),  # R, G, Q
    "Mg": np.array([2.44, 1.76, 0.32]),
    "S":  np.array([1.15, 0.56, 0.39]),
}

RGQ_meas_err = {
    "Si": np.array([0.45, 0.07, 0.04]),  # R, G, Q uncertainties
    "Mg": np.array([0.46, 0.17, 0.09]),
    "S":  np.array([0.30, 0.08, 0.06]),
}
...
def R_analytic_local(Z, N_li, N_he, v, pi, nele, mixing, gamma):
    ...
    #____
    # Utilde - term describing contribution from y photoexcitation 
    #Line profile
    ...
    Utilde = (B_y * Phi_y / (x_i * tot_beta_He)) * (1 - P_rad_y) #RAD-modified
	...
	#Decay rate from z to ground    
    He_z_params = get_params_He(Z, 'z')
    Azg = He_z_params[2]
    
    #Critical photoexcitation rate
    phi_c = Azg / (1 + F + S + U) #s-1
    
    #RAD-modified UV photoexcitation rate
    ...
    Xtilde = Btilde * (phi_UV_tilde + n_e * C_zi[1]) #RAD-modified
    Y = (B / Azg) * (phi_UV - phi_UV_tilde)
    
    Bz = 1 - B
    R_0_prime = (F + Bz + S) / B
    #R = R_0_prime * B / (Btilde * ((1 + (phi_UV_tilde / phi_c) + (n_e / n_c)) + Y))
    R = ((R_0_prime + U) * B) / (Utilde * (1 + X) + Btilde * ((1 + (phi_UV_tilde / phi_c) + (n_e / n_c)) + Y))

    #R_old = (R_0 + S/B) / (((Btilde / B) * (1 + (phi_UV_tilde / phi_c) + (n_e / n_c))) + S * Xtilde / (B * Azg))
    
    return R
    
#Calculates observed (global) R ratio averaged over entire medium factoring in plasma geometry, observer position, inclination (FULL APPROX)
#Assuming simple, truncated cone for geometry
#Assumming cold photoionized plasma (ne << nc, phi_UV << phi_c) 
def R_analytic_global(Z, v, mixing, gamma, alpha, R0, R, L, beta, xi_max, inc):
	...
	#Voigt profiles for He-like x,y; Li-like s,t
    phi_x = voigt(e_grid, x_centroid_energy, sigma_em_x, x_lorentz_gamma)
    phi_y = voigt(e_grid, y_centroid_energy, sigma_em_y, y_lorentz_gamma)
    phi_s = voigt(e_grid, s_centroid_energy, sigma_em_s, s_lorentz_gamma)
    phi_t = voigt(e_grid, t_centroid_energy, sigma_em_t, t_lorentz_gamma)
    ...
    r_lin = np.arange(R0, R, 0.5 * 3.086e+18) #radial distribution along LoS
    ...
    Froot_he = F_root_bimodal(Z=Z, nele=2, mixing=mixing, L=L, r0=R0, r=r_grid, beta=beta, xi_max=xi_max)
    
    if np.max(Froot_he) <= 1e-15:
        raise ValueError("Not enough He-like for calculation")
    
    #Calculate n_he as a function of r
    n_r_he = partial(get_n, Z=Z, nele = 2, mixing=mixing, L=L, r0=R0, beta=beta, xi_max=xi_max)
    n_he = n_r_he(r_grid)
    ...
    #Li-like
    #Calculate n_he as a function of r
    n_r_li = partial(get_n, Z=Z, nele = 3, mixing=mixing, L=L, r0=R0, beta=beta, xi_max=xi_max)
    n_li = n_r_li(r_grid)
    ...
    for i in range(len(theta_lin)):
        for j in range(len(phi_lin)):
            for k in range(len(r_lin)):
                tau = obs_tau(r_grid[i,j,k], theta_grid[i,j,k], phi_grid[i,j,k], inc, alpha, sigma_abs, n_r_li)
                #tau_y = obs_tau(r_grid[i,j,k], theta_grid[i,j,k], phi_grid[i,j,k], inc, alpha, sigma_y, n_r_he)
    ...
    bar_beta_He = simpson(simpson(simpson((1 / (r_grid/R0)**(beta)) * np.sin(theta_grid) * Froot_he, theta_lin, axis = 0), phi_lin, axis = 0), r_lin, axis = 0)
    Btilde_y = B_y * (1 - P_rad_y)
    Btilde_x = B_x * (1 - P_rad_x) 
    Btilde = (1./3.) * Btilde_y + (5./9.) * Btilde_x

    #RAD-modified total photoionization rate (integrated)
    tilde_beta_He = tot_beta_He * simpson(simpson(simpson(Froot_he * (R0/r_grid)**(beta) * np.sin(theta_grid) * (Btilde / B) * (1 + Utilde), theta_lin, axis=0),phi_lin,axis=0),r_lin,axis=0)
    #tilde_beta_He = tot_beta_He * simpson(simpson(simpson(10**(Froot_he) * (R0/r_grid)**(beta) * np.sin(theta_grid) * (Btilde / B), theta_lin, axis=0),phi_lin,axis=0),r_lin,axis=0)
    if (tilde_beta_He == 0):
...
#Assumming cold photoionized plasma (ne << nc, phi_UV << phi_c) 
def Q_analytic_global(Z, v, mixing, gamma, alpha, R0, R, L, beta, xi_max):
...
def G_analytic_local(Z, N_li, N_he, v, pi, nele, mixing, gamma):
  ...
  #____
    ...
    G = (1 / K) * (Upsilon / (Upsilon + 1)) * (Btilde * (1 + Utilde) + (F + S + Bz) * (1 + Xtilde) / (1 + X))

    return G

    
#Calculates observed (global) G ratio averaged over entire medium factoring in plasma geometry, observer position, inclination (FULL APPROX)
#Assuming simple, truncated cone for geometry
#Assumming cold photoionized plasma (ne << nc, phi_UV << phi_c) 
def G_analytic_global(Z, v, mixing, gamma, alpha, R0, R, L, beta, xi_max, inc):
  ...
  #____
    ...
    dPhi_w = (1 - np.exp(-1*tau_w))
    Phi_w = I_w * simpson(dPhi_w, e_grid_w)
...
#Calculates all three ratios simultaneously for the same input params
def get_RGQ_global(Z, v, mixing, gamma, alpha, R0, R, L, beta, xi_max, inc):
    R_ratio = R_analytic_global(Z, v, mixing, gamma, alpha, R0, R, L, beta, xi_max, inc)
    print(R_ratio)
    G_ratio = G_analytic_global(Z, v, mixing, gamma, alpha, R0, R, L, beta, xi_max, inc)
    print(G_ratio)
    Q_ratio = Q_analytic_global(Z, v, mixing, gamma, alpha, R0, R, L, beta, xi_max)
    print(Q_ratio)
    return np.array([R_ratio,G_ratio,Q_ratio])
 
 
#Calculates chi-square after inputting measured RGQ ratios from observed spectra   
def chi_sq_RGQ_global(Z, v, mixing, gamma, alpha, R0, R, L, beta, xi_max, inc, RGQ_meas, RGQ_meas_err):
    RGQ_model = get_RGQ_global(Z, v, mixing, gamma, alpha, R0, R, L, beta, xi_max, inc)
...
def chi_for_min_global(x, Z, mixing, gamma, alpha, L, inc, RGQ_meas, RGQ_meas_err):
    print(x)
    vel = x[0]
    #mixing = x[1]
    R0 = x[1]
    ...
    RGQ_meas = np.array(RGQ_meas)
    RGQ_meas_err = np.array(RGQ_meas_err)
    chi = chi_sq_RGQ_global(Z, vel, mixing, gamma, alpha, R0, R, L, beta, xi_max, inc, RGQ_meas, RGQ_meas_err)
    print(chi)
    return chi


def chi_for_min_global_multi(x, Z, gamma, alpha, L, inc,RGQ_meas, RGQ_meas_err):

    if not isinstance(Z, dict):
        raise TypeError(f"Z must be a dict, got {type(Z).__name__}")

    if not isinstance(RGQ_meas, dict):
        raise TypeError(f"RGQ_meas must be a dict, got {type(RGQ_meas).__name__}")

    if not isinstance(RGQ_meas_err, dict):
        raise TypeError(f"RGQ_meas_err must be a dict, got {type(RGQ_meas_err).__name__}")

    if set(RGQ_meas.keys()) != set(RGQ_meas_err.keys()):
        raise ValueError("RGQ_meas and RGQ_meas_err must contain the same element keys")
        
    if set(Z.keys()) != set(RGQ_meas.keys()):
        raise ValueError("Z, RGQ_meas, and RGQ_meas_err must contain the same element keys")
    ...
    for elem in RGQ_meas:
        print(elem)
        chi = chi_sq_RGQ_global(Z[elem], vel, mixing, gamma, alpha, R0, R, L, beta, xi_max, inc, RGQ_meas[elem], RGQ_meas_err[elem])
        chi_tot += chi
    print(chi_tot)
    return chi_tot


def solve_RGQ_global(Z, gamma, alpha, L, inc, RGQ_meas, RGQ_meas_err):
    #res = minimize(chi_for_min_global,x0=[250, 0.5, 0.4, 1.5, 10.0],args=(Z, mixing, gamma, alpha, L, inc, RGQ_meas,RGQ_meas_err),bounds=[(100.1,399.9), (0.01, 0.99), (0.31, 0.51), (1.01, 1.99),(9.01, 11.01)])
    res = minimize(chi_for_min_global_multi,x0=[260, 0.53, 0.478, 1.73, 8.2],args=(Z, gamma, alpha, L, inc, RGQ_meas,RGQ_meas_err),bounds=[(100.1,399.9), (0.01, 0.99), (0.31, 0.51), (1.01, 1.9),(6.01, 10.01)])
    return res
